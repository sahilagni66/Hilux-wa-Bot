...
System({
    ...
    await heroku.get(baseURI + "/formation").then(async (formation) => {
        await message.send(` *-Hilux is shutting down-* `);
...
async (message, match) => {
    if (!match)
        return await message.send(`Example: .setvar SUDO:918891005819`);   
...
async (message, match) => {
    ...
    if (match === "now") {
        ...
        if (commits.total === 0) {
            return await message.send(`_Hilux on the latest version: v${version}_`);
        } else {
            await message.reply("  -Hilux is updating- ");
    ...
    if (commits.total === 0) {
        ...
        var availupdate = "*𝘜𝘱𝘥𝘢𝘵𝘦𝘴 𝘈𝘳𝘦 𝘈𝘷𝘪𝘭𝘢𝘣𝘭𝘦* \n\n";
